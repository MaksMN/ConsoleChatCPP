...
User::User(const uint &id, const std::wstring &login, const std::wstring &name, std::wstring &pass)
    : _id(id),
      _login(login),
      _name(name),
...
void User::setName(const std::wstring &name)
{
    _name = name;
}

std::wstring User::getLogin()
{
    return _login;
}
...
void User::printData()
{
    std::wcout << _name << L"[" << _login << L"] [userid " << _id << L"] ";
    if (isAdmin())
        std::wcout << L"[group admin] ";
    if (isBanned())
        std::wcout << L"[status banned] ";
    if (isUser())
        std::wcout << L"[group user] ";

    std::wcout << L"Рег." << StampToTime(_timestamp) << std::endl;
    std::wcout << std::endl;
}

void User::setPass(std::wstring &pass)
{
    setSalt(abs(pass[0]));
...
bool User::validatePass(std::wstring &pass)
{
    char message[_pass_bytes];
    bytesForHash(pass, message);
...
void User::bytesForHash(const std::wstring &pass, char message[])
{
